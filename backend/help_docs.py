...
- 匯出後「可能跳對話框、可能直接下載」的分歧用 `if_element_found`（UIA 版條件分支）
  ...
  **通常不用**手動加喚醒動作。仍想明確控制喚醒時機(例如流程一開始就喚醒、
  ...
  何時仍要手動加喚醒(判斷規則):流程碰該視窗的**第一個動作**是
  「填值/點擊/讀值」→ 不用加(自動救援會兜底);是「uia_wait 等待、
  if_* 條件探測、CV 點擊、OCR 讀字」→ 前面加 activate_window+wait ——
  等待/探測對睡死的視窗會誤判(空樹被當成「元素不在」走錯分支),
  CV/OCR 則需要視窗實際顯示在螢幕上、不會自動被叫上來。
