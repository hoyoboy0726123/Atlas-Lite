...
每次執行建立獨立的 .log 檔，記錄完整的 subprocess 輸出與驗證結果。
...
def resolve_log_dirs() -> list[Path]:
    """回傳要搜尋執行 log 的目錄（只回存在的）。

    讀 log 的各處都必須用這個、不可自己寫死路徑 —— Atlas 就是因為多處寫死
    `backend/ai_output/pipeline_logs`，而 logger 實際寫到別處，導致新的執行
    一律找不到 log。
    """
    return [LOG_DIR] if LOG_DIR.exists() else []
